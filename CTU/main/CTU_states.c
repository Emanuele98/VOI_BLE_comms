...
extern const ble_uuid_t *wpt_char_dyn_uuid;
...
static void CTU_configuration_state(void *arg)
{
    ...
    if (( ((now > conf_time + CONF_STATE_TIMEOUT) /*&& (peer_get_NUM_AUX_CTU())*/) || ((peer_get_NUM_AUX_CTU() == 2) && (all_AUX_CTU_set())) ) && (!TOO_COLD) )
...
static void CTU_local_fault_state(void *arg)
{
    ...
    else if(CTU_is_charging() || CTU_has_fully_charged())
...
static void CTU_remote_fault_state(void *arg)
{
    ...
    if (!CTU_is_charging() && !CTU_has_fully_charged())
        CTU_state_change(CTU_LOW_POWER_STATE, (void *)peer);
    else 
        CTU_state_change(CTU_POWER_TRANSFER_STATE, (void *)peer);
}
